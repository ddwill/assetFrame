...
public class ScencesManager{
	...
	private static EditorScenceItemData LoadEditorScenceItemData(EditorScenceItemData esid)
	{
		...
		if (esid.isNeedSave) {
			...
			string path = ConfigPath.TrimPath_Resource(esid.Path);
			if(esid.parentPrefabPath!=null) path = ConfigPath.TrimPath_Resource(esid.parentPrefabPath);
			path = ConfigPath.TrimSuffix(path);
